...
contract ICO_Contract {
     ...
     * @param encBlob Encrypted bid data (price, quantity, pitch, country)
     *                Only the TEE can decrypt this using its private key
     * @param maxSpend Maximum USDC amount authorized for spending via permit
     * @param permitSig EIP-2612 permit signature allowing gasless USDC transfers
     *                  This eliminates the need for separate approve() transactions
     */
    function submitBid(
        uint256 saleId,
    ...
    function finalize(
        ...
        // This is the critical security check that proves the settlement results
        // were generated by the authorized TEE agent and not tampered with.
        // The TEE signs: keccak256(contract_address, sale_id, settlement_data)
